...
\begin{figure}[h]
	\centering
	\centering
		\includegraphics[scale=0.4]{schemat_ann}	
...
%napisz coś o normalizacji batcha, bo to też jest używana warstwa i o dropoucie. Przynajmniej trochę, więcej można w rodziale 3 o przygotowaniu danych i zapobieganiu overfittingowi
...
\item Struktura sieci (DODAĆ TUTAJ TABULATOR DLA TEJ TABELI CZY CO TO JEST):
\end{itemize}

\newsavebox\myvv
\begin{lrbox}{\myvv}\begin{minipage}{1.0\textwidth}
\begin{verbatim}
...
%W warstwach konwolucyjnych użyto funkcji aktywacyjnej ReLu, bez dopełniania ramek zerami na brzegach (ang. \textit{padding - same}) i wielkością filtra zmieniającą się od 1 do 3. Warstwy normalizacji wsadowej mają parametr bnmomentum równy 0.85 (CO TO ZNACZY?!). Użyte warstwy redukcyjne z funkcją maksymalizującą miały parametr pool_size równy 2 (A TO CO ZNACZY?). Sieć zakończono typowym klasyfikatorem stosowanym w modelach do problemów niebinarnych. 

Przyjęty optymizator (CZYLI CO?) to RMSprop, szybkość uczenia to 2e-5, funkcja liczenia błędu to .... (ang. \textit{binary crossentropy}). Uczenie trwało 25 epok, każda epoka trwała 310 iteracji, a walidacja 62 iteracje.
...
Znaczącą różnicą jest zastosowanie klasyfikatora pięciowyjściowego, co pozwala na przypisanie do ramki klasy "żadna".

\end{itemize}
\newsavebox\myvf
\begin{lrbox}{\myvf}\begin{minipage}{1.0\textwidth}
\begin{verbatim}
_________________________________________________________________
Layer (type)                 Output Shape              Param #   
_________________________________________________________________
...
\end{verbatim} 
\end{minipage}\end{lrbox}
\resizebox{0.75\textwidth}{!}{\usebox\myvf}

\begin{itemize}
\item Parametry treningu:

Model był trenowany przez 30 epok.
\item Wyniki eksperymentu:
